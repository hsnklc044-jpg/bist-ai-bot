...
print("AI TRADER V9 ELITE CALISIYOR...")
send("🧠 AI TRADER V9 ELITE AKTİF")

while True:
    try:
        now = datetime.now()

        # 🕐 Session filtresi (10:00 - 18:00)
        ...
        for c in sorted(candidates, key=lambda x: x[3], reverse=True):
            ...
            lot = round(risk_amount / (price - sl), 2)
            ...
            send(f"🚀 {trade['symbol']} Entry:{round(price,2)} Lot:{lot}")
            log(f"OPEN {trade}")
