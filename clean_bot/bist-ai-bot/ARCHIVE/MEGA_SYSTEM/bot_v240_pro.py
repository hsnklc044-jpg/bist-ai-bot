...
print("🚀 RUNNING V240 PRO (SMART FILTERS)")
...
def analyze(s):
    ...
    ema_strength = (0.0015 - ema_gap) * 1200
    momentum_strength = abs(momentum) * 800
    body_strength = body * 1000
...
def execute_trade(s,side,score,tier):
    ...
    tp,sl=tp_sl(s,p,side,score,tier)
    if not tp:
        return

    send(f"""🚀 V240 TRADE ({tier})
{s} {side}
