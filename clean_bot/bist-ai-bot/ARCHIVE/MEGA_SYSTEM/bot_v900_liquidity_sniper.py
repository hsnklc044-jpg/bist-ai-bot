...
print("🚀 RUNNING V900 LIQUIDITY SNIPER")
...
def analyze(s):
    ...
    # PULLBACK ENTRY
    pullback = abs(c5[-1] - c5[-2]) / c5[-2]
    if pullback < 0.001:
        return None

    if side == "SHORT" and c5[-1] > c5[-2]:
        return None
    if side == "LONG" and c5[-1] < c5[-2]:
        return None

    # MICRO STRUCTURE
    last3 = c5[-3:]
    if side == "SHORT" and not (last3[0] > last3[1] > last3[2]):
        return None
    if side == "LONG" and not (last3[0] < last3[1] < last3[2]):
        return None

    # 🔥 LIQUIDITY SWEEP
    last_high = h5[-1]
    last_low  = l5[-1]
    prev_high = max(h5[-5:-1])
    prev_low  = min(l5[-5:-1])

    if side == "SHORT" and last_high < prev_high:
        return None

    if side == "LONG" and last_low > prev_low:
        return None
    ...
    # VOL TRAP
    if vol_pct < 0.0008 or vol_pct > 0.005:
        return None
...
# ================= MAIN =================
def main():
    send("🧠 V900 LIQUIDITY SNIPER STARTED")
