...
print("🚀 RUNNING V250 DUAL ENGINE")
...
def analyze(s):
    ...
    if recent_range < 0.002 and abs(momentum) > 0.0004:
        score = (0.002 - recent_range)*1500 + abs(momentum)*800

        if score > MIN_SCORE:
            print(f"{s} ⚡ SQUEEZE {dir5} score={round(score,2)}")
            return dir5,score,"SQUEEZE"
    ...
    if trend_strength > 0.001:
        ...
        if dir5=="LONG" and c5[-1] < c5[-2]:
            return None,0,None
        if dir5=="SHORT" and c5[-1] > c5[-2]:
            return None,0,None

        score = trend_strength*2000 + abs(momentum)*500

        if score > MIN_SCORE:
            print(f"{s} 📈 TREND {dir5} score={round(score,2)}")
            return dir5,score,"TREND"

    return None,0,None

# ================= TP SL =================
def tp_sl(s,p,side,mode):
...
# ================= EXECUTE =================
def execute_trade(s,side,score,mode):

    p=price(s)
    if not p:
        return

    tp,sl=tp_sl(s,p,side,mode)
    if not tp:
        return

    send(f"""🚀 V250 TRADE ({mode})
{s} {side}
