...
print("V39 SMART AI CALISIYOR...")
send("🧠 V39 SMART LOT AI AKTİF")
...
while True:
    try:
        ...
        if best_trade and len(active_trades) < MAX_TRADES:
            ...
            send(f"🚀 TRADE {s} Lot:{best_trade['lot']} Risk:{round(dynamic_risk*100,2)}%")
        ...
        for t_ in active_trades[:]:
            ...
            if price >= t_["tp"]:
                ...
                send(f"🟢 TP {t_['symbol']} +{round(profit,2)} | WinStreak:{win_streak}")
                ...
                send(f"🔴 SL {t_['symbol']} -{round(loss,2)} | LossStreak:{loss_streak}")
