...
print("ELIT PRO CALISIYOR...")

send("🚀 ELIT PRO AKTİF")

while True:
    try:
        msg = "🧠 ELIT PRO SİNYALLER:\n\n"
        ...
        for symbol in symbols:
            ...
            if rsi_val < 40 and price > ma20:
                elite.append((symbol, price, rsi_val, tp, sl))

            elif rsi_val < 50 and price > ma20:
                normal.append((symbol, price, rsi_val, tp, sl))

            elif rsi_val < 60:
                agresif.append((symbol, price, rsi_val, tp, sl))
        ...
        if normal:
            msg += "\n⚡ NORMAL:\n"
            for n in normal:
                msg += f"{n[0]} | {round(n[1],2)} | RSI:{round(n[2],2)}\n"

        if agresif:
            msg += "\n🔥 AGRESİF:\n"
            for a in agresif:
                msg += f"{a[0]} | {round(a[1],2)} | RSI:{round(a[2],2)}\n"
