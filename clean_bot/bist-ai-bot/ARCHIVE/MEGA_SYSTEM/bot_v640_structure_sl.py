...
print("🚀 RUNNING V640 STRUCTURE SL")
...
def execute(s,side,score,vol,vol_pct,recent_low,recent_high):
    ...
    # 🔥 STRUCTURE + VOL SL
    if side=="LONG":
        sl = min(p - sl_vol, recent_low)
    else:
        sl = max(p + sl_vol, recent_high)
...
Entry:{p}
SL:{round(sl,4)}
Structure:{round(recent_low if side=='LONG' else recent_high,4)}
