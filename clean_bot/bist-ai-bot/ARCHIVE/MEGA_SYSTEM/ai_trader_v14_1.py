...
print("AI TRADER V14.1 CALISIYOR...")
send("🧠 V14.1 SMART FILTER AKTİF")
...
while True:
    try:
        ...
        for c in sorted(candidates, key=lambda x: x[3], reverse=True):
            ...
            send(f"🚀 V14.1 TRADE {c[0]} Score:{c[3]} Entry:{round(price,2)} TP:{tp} SL:{sl}")
        ...
        for t in active_trades[:]:
            ...
            if price >= t["tp"]:
                send(f"🟢 WIN {t['symbol']} Score:{t['score']}")
                active_trades.remove(t)

            elif price <= t["sl"]:
                send(f"🔴 LOSS {t['symbol']} Score:{t['score']}")
                active_trades.remove(t)
