...
try:
    with open(log_file, "x", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["time","symbol","entry","tp","sl","result","profit"])
except:
    pass

def log_trade(symbol, entry, tp, sl, result, profit):
    with open(log_file, "a", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([datetime.now(), symbol, entry, tp, sl, result, profit])
...
print("AI TRADER V22 LOG SYSTEM CALISIYOR...")
send("🧠 V22 LOG SYSTEM AKTİF")
...
while True:
    try:
        ...
        for t in active_trades[:]:
            ...
            if price >= t["tp"]:
                profit = (t["tp"] - t["entry"]) * 1000
                capital += profit
                wins += 1
                total_profit += profit

                log_trade(t["symbol"], t["entry"], t["tp"], t["sl"], "WIN", profit)

                send(f"🟢 WIN {t['symbol']} +{round(profit,2)}")
                active_trades.remove(t)

            elif price <= t["sl"]:
                loss = (t["entry"] - t["sl"]) * 1000
                capital -= loss
                losses += 1
                total_profit -= loss

                log_trade(t["symbol"], t["entry"], t["tp"], t["sl"], "LOSS", -loss)
