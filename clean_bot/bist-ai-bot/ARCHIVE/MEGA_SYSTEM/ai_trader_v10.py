...
print("AI TRADER V10 CALISIYOR...")
send("🧠 AI TRADER V10 AKTİF")
...
while True:
    try:
        ...
        for c in sorted(candidates, key=lambda x: x[3], reverse=True):
            ...
            lot = min(lot, round((capital * 0.35) / price, 2))
        ...
        for t in active_trades[:]:
            ...
            if price >= t["tp"]:
                ...
                log_trade(f"TP {t} PROFIT:{profit}")

                COOLDOWN[t["symbol"]] = time.time()
                active_trades.remove(t)

            elif price <= t["sl"]:
                result = (price - t["entry"]) * t["lot"]
                capital += result
                total_profit += result

                if result >= 0:
                    wins += 1
                    send(f"🟢 EXIT {t['symbol']} +{round(result,2)}")
                    log_trade(f"EXIT WIN {t} PROFIT:{result}")
                else:
                    losses += 1
                    send(f"🔴 SL {t['symbol']} {round(result,2)}")
                    log_trade(f"SL {t} LOSS:{result}")
