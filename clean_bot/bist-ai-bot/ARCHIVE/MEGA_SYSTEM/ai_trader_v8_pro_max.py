...
print("AI TRADER V8 PRO MAX CALISIYOR...")
send("🧠 AI TRADER V8 PRO MAX AKTİF")

while True:
    try:
        dd = (initial_capital - capital) / initial_capital
        if dd >= MAX_DRAWDOWN:
            send("⛔ MAX DRAWDOWN - SİSTEM DURDU")
            break
        ...
        for symbol in symbols:
            ...
            # Volatilite filtresi (adaptif)
            vol_ratio = atr_val / price
            if vol_ratio > 0.06:
                continue

            if score >= 4:
                candidates.append((symbol, price, score, atr_val))

        # ❗ No trade spam engelle
        if len(candidates) == 0:
            if time.time() - last_signal_time > 600:
                send("⚖️ Piyasa uygun değil")
        ...
        for trade in active_trades[:]:
            ...
            if trade["max_price"] >= trade["entry"] * (1 + TRAIL_TRIGGER):
                ...
                if new_sl > trade["sl"]:
                    trade["sl"] = new_sl
                    send(f"🔄 SL Update {trade['symbol']} → {round(trade['sl'],2)}")
