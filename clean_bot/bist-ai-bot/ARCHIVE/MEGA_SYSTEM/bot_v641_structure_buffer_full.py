...
print("🚀 RUNNING V641 STRUCTURE + BUFFER")
...
# ================= EXECUTE =================
def execute(s,side,score,vol,vol_pct,recent_low,recent_high):
    ...
    sl_vol = p * vol_pct * multiplier

    # 🔥 BUFFER (score bazlı)
    buffer = p * (0.0015 if score>4 else 0.001)

    # 🔥 STRUCTURE + BUFFER
    if side=="LONG":
        sl = min(p - sl_vol, recent_low - buffer)
        structure_val = recent_low
    else:
        sl = max(p + sl_vol, recent_high + buffer)
        structure_val = recent_high
...
Entry:{p}
SL:{round(sl,4)}
Structure:{round(structure_val,4)}
...
def manage():
    ...
    for s,t in list(state.items()):
        ...
        # PARTIAL
        if not t["partial"]:
            if t["side"]=="LONG" and p >= entry*1.012:
                t["partial"]=True
                send(f"💰 PARTIAL {s}")
            if t["side"]=="SHORT" and p <= entry*0.988:
                t["partial"]=True
                send(f"💰 PARTIAL {s}")

        # TRAILING
        multiplier = 2.5 if t["score"]>4 else 1.8
...
def run():
    ...
    for s in SYMBOLS:
        ...
        sig,score,vol,vol_pct,low,high=analyze(s)

        if not sig:
            continue

        if not best or score>best[2]:
            best=(s,sig,score,vol,vol_pct,low,high)
...
# ================= MAIN =================
def main():
    send("🧠 V641 STRUCTURE BUFFER STARTED")
