...
def analyze(symbol):
    ...
    threshold = 45 if not in_position else 35
    ...
    try:
        ...
        if best:
            send_telegram(
                ...
                f"Trend:{round(best['trend'],2)}\nMomentum:{round(best['momentum'],2)}\n"
                f"Hacim:{round(best['volume'],2)}\nVolatilite:{round(best['volatility'],2)}"
            )
