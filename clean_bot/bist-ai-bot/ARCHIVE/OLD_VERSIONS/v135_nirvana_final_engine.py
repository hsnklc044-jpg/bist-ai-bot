    ...
    while True:
        ...
        for sym, sig, strength, score in candidates:
            ...
            if bias and sig != bias:
                ...
                if strength < 0.00045:
                    print(sym, "BLOCK: weak vs bias")
                    continue
                else:
                    print(sym, "OVERRIDE bias 🚀")
        ...
        # ===== EN İYİSİNİ SEÇ =====
        best = max(filtered, key=lambda x: x[3])

        sym, sig, strength, score = best

        print("BEST SIGNAL:", sym, sig, "SCORE:", round(score,6))

        # ===== COOLDOWN =====
        last_time = state.get(sym, {}).get("time", 0)
        last_side = state.get(sym, {}).get("side")
