...
def log_trade(row):
    ...
    with open(TRADE_LOG, mode="a", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow(["time","symbol","side","entry","exit","pnl"])
        writer.writerow(row)
...
# ================== MAIN ==================
print("🚀 ULTRA PRO TRADE ENGINE STARTED")
send_telegram("🧠 ULTRA PRO TRADE ENGINE STARTED")
    ...
    try:
        ...
        if position == "LONG" and sym == entry_symbol:
            ...
            if price < trailing_price - TRAIL_ATR * entry_atr:
                pnl = price - entry_price
                total_pnl += pnl
                print(f"📉 TRAILING EXIT LONG: {pnl:.2f}")
                log_trade([datetime.now(), sym, "LONG", entry_price, price, pnl])
                position = None
                trailing_price = None
            ...
            if price > trailing_price + TRAIL_ATR * entry_atr:
                pnl = entry_price - price
                total_pnl += pnl
                print(f"📉 TRAILING EXIT SHORT: {pnl:.2f}")
                log_trade([datetime.now(), sym, "SHORT", entry_price, price, pnl])
                position = None
                trailing_price = None
        ...
        elif side == "SELL" and position is None:
            position = "SHORT"
            entry_price = price
            entry_symbol = sym
            entry_time = datetime.now()
