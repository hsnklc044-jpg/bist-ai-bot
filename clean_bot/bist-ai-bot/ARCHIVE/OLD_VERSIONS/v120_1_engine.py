...
MIN_PRICE_DIFF = 0.002  # %0.2 fark olmadan tekrar sinyal yok
...
# ANTI-SPAM (FINAL)
# =========================
def is_new_signal(symbol, side, entry):
    global last_signals

    if symbol in last_signals:
        last_side = last_signals[symbol]["side"]
        last_entry = last_signals[symbol]["entry"]

        price_diff = abs(entry - last_entry) / last_entry

        if last_side == side and price_diff < MIN_PRICE_DIFF:
            return False
...
# =========================
def main():
    print("🚀 v120.1 PRO ENGINE STARTED")
