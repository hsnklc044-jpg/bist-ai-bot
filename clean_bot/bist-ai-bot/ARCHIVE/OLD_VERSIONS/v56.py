...
def manage_trade(symbol, price):
    ...
    if not t["be"] and price >= t["entry"] * 1.01:
        t["sl"] = t["entry"]
        t["be"] = True
        send_telegram(f"🔒 BE AKTİF {symbol}")

    # TRAILING (%1.5 geriden takip)
    if t["be"]:
        new_sl = price * 0.985
        if new_sl > t["sl"]:
            t["sl"] = new_sl
    ...
    if price >= t["tp"]:
        ...
        send_telegram(f"❌ SL/TRAIL {symbol}")
        del OPEN_TRADES[symbol]

# ================= RUN =================
def run():
    send_telegram("🚀 V56 TRAILING AKTİF")
    ...
    while True:
        try:
            for symbol in COINS:
                ...
                if score >= 50 and (r < 75 or (r < 80 and t1h)) and t5 and t15 and t1h:
                    if symbol not in OPEN_TRADES:
