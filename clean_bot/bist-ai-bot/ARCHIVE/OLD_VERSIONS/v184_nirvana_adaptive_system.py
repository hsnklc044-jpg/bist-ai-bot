...
# ================= MAIN =================
def run():
    print("🚀 v184 ADAPTIVE SYSTEM")
    send("✅ BOT STARTED v184")
    ...
    while True:
        ...
        if vol < 0.003:
            MIN_SCORE = BASE_MIN_SCORE * 0.8   # daha agresif
            STRONG_SCORE = BASE_STRONG_SCORE * 0.9
        else:
            MIN_SCORE = BASE_MIN_SCORE
            STRONG_SCORE = BASE_STRONG_SCORE * 1.1  # daha seçici
        ...
        for s in SYMBOLS:
            if s in state:
                continue
            sig,score,v = analyze(s)
            if sig:
                cands.append((s,sig,score,v))
        ...
        if elite:
            ...
            s,sig,score,v = elite[0]

            if not current_side or sig == current_side:
                entry = price(s)
                if entry:
                    tp,sl = tp_sl(entry,sig,v)
