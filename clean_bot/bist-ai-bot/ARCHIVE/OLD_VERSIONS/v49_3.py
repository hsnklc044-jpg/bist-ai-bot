...
def calculate_score(df):
    ...
    return score, trend, momentum, volume_score, volatility

def analyze(symbol):
    df = get_data(symbol)
    score, trend, momentum, vol_score, volatility = calculate_score(df)
...
print("V49.3 AI TRUE ENGINE BASLADI...")

while True:
    try:

        if time.time() - last_heartbeat > 60:
            send_telegram("🤖 AI aktif | tarama devam")
        ...
        # 🔥 HER ZAMAN GÖSTER
        if best and time.time() - last_debug > 60:
            send_telegram(
                f"📊 BEST\n{best['symbol']}\nScore:{round(best['score'],2)}"
            )
            last_debug = time.time()

        # 🔥 TRADE SADECE EŞİK ÜSTÜ
        threshold = 35 if not in_position else 30

        if not in_position and best and best["score"] >= threshold:
            ...
            if ok:
                ...
                send_telegram(f"🚀 AI ENTRY {symbol} Score:{round(best['score'],2)}")
            else:
                in_position = False
