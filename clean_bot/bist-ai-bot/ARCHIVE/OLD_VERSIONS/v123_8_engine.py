...
# =========================
# SMART FILTER (SOFT LIMIT)
# =========================
def can_trade(sym,side,entry,closes):
    now=time.time()

    # trend gücü (EMA farkı / fiyat)
    e9 = ema(closes,9)
    e21 = ema(closes,21)
    ...
    if active_count >= BASE_MAX_TRADES:
        if strength < 0.002:
            print("BLOCK: max trades (weak)")
            return False
        else:
            print("OVERRIDE: strong trend (max trades)")

    # SAME DIRECTION (soft)
    if same_dir >= BASE_MAX_SAME:
        if strength < 0.002:
            print("BLOCK: same direction (weak)")
            return False
        else:
            print("OVERRIDE: strong trend (same direction)")

    # COOLDOWN + yakın fiyat filtresi
    if sym in state:
        s=state[sym]
...
def process(sym):
    ...
    if not can_trade(sym,sig,price,closes):
        return
