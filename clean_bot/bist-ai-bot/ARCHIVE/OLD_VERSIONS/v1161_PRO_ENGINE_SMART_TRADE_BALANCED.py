...
def get_signal(df):
    ...
    # 🔥 TREND (yumuşatıldı)
    trend = ma_short > ma_mid

    # 🔥 MOMENTUM (2 mum yeterli)
    last2 = close.iloc[-2:].values
    momentum = last2[1] > last2[0]

    # 🔥 HACİM (daha gerçekçi)
    vol_now = volume.iloc[-1].item()
    vol_avg = volume.rolling(10).mean().iloc[-1].item()
    volume_ok = vol_now > vol_avg * 1.2

    if trend and momentum and volume_ok:
        return "BUY", price
...
print("🚀 PRO SMART TRADE BALANCED STARTED")
