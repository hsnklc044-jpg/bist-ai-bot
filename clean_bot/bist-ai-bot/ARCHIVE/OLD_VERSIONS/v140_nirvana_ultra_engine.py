...
def analyze(symbol):
    ...
    if strength > 0.0006:
        return trend, strength * 3

    if trend == "LONG" and r < 60:
        return "LONG", strength * (60 - r)

    if trend == "SHORT" and r > 40:
        return "SHORT", strength * (r - 40)
...
def update_trailing(trade, price):
    ...
    if pnl > TRAIL_START:
        if side == "LONG":
            ...
            if new_sl > trade["sl"]:
                trade["sl"] = new_sl
                print("TRAIL LONG UPDATE")
        else:
            new_sl = price * (1 + TRAIL_GAP/100)
            if new_sl < trade["sl"]:
                trade["sl"] = new_sl
                print("TRAIL SHORT UPDATE")
...
def run():
    ...
    print("🚀 v140 NIRVANA ULTRA ENGINE")
