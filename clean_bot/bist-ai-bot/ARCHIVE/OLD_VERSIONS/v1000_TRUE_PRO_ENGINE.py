...
def market_filter():
    try:
        df = yf.download("XU100.IS", period="5d", interval="5m", progress=False)
        if df is None or df.empty:
            return False
...
while True:
    try:
        ...
        for sym in SYMBOLS:
            ...
            # CORRELATION FILTER
            same_side = [p for p in positions.values() if p["side"] == side]
            if len(same_side) >= 1:
                continue
