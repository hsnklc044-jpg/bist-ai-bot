...
# ================= PULLBACK LOGIC =================
def check_pullback(symbol, r):

    if symbol not in PULLBACK_STATE:
        PULLBACK_STATE[symbol] = {"overbought": False}

    state = PULLBACK_STATE[symbol]

    # önce aşırı alım gör
    if r > 70:
        state["overbought"] = True

    # sonra geri çekilme
    if state["overbought"] and 45 < r < 60:
...
# ================= RUN =================
def run():
    send_telegram("🚀 V57 PULLBACK AKTİF")
