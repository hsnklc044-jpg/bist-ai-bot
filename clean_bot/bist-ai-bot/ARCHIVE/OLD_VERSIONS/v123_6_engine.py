...
# =========================
# TREND STRENGTH (SMART)
# =========================
def trend_strength(c):
    e9=ema(c,9)
    e21=ema(c,21)
    diff=abs(e9[-1]-e21[-1])/c[-1]
    return diff
...
# FILTER (SMART)
# =========================
def can_trade(sym,side,entry,c):
    now=time.time()

    # 🔥 trend bazlı limit
    strength = trend_strength(c)

    max_trades = BASE_MAX_TRADES
    max_same = BASE_MAX_SAME

    if strength > 0.003:  # güçlü trend
        max_trades += 2
        max_same += 1
    ...
    same_dir = sum(
        1 for t in state.values()
        if t.get("active") and t.get("side") == side
    )
...
def process(sym):
    ...
    if not can_trade(sym,sig,price,c5):
        return
