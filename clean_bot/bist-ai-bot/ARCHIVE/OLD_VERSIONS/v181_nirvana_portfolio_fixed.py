...
# ================= MAIN =================
def run():
    print("🚀 v181 PORTFOLIO FIXED")

    send("✅ BOT STARTED v181")
...
Score:{round(score,5)} RR:{round(rr,2)}""")
        ...
        # ================= NORMAL =================
        # 🔥 SADECE ELITE VARSA NORMAL AÇ
        if elite and len(state) < MAX_TRADES and normal:
...
Score:{round(score,5)} RR:{round(rr,2)}""")
        ...
        # 🔥 ELITE YOKSA SADECE 1 NORMAL
        if not elite and not state and normal:
            normal.sort(key=lambda x:x[2],reverse=True)
