...
def analyze(symbol, btc_direction):
    ...
    print(f"{symbol} | RSI:{round(rsi_val,2)} | VOL:{vol_ok} | MOM_UP:{mom_up} | MOM_DOWN:{mom_down} | TREND_UP:{trend_up} | TREND_DOWN:{trend_down}")

    if btc_direction == "UP":
        if not (trend_up and mtf_up and mom_up and 48 < rsi_val < 68 and vol_ok):
            return None
        direction = "LONG"

    elif btc_direction == "DOWN":
        if not (trend_down and mtf_down and mom_down and 32 < rsi_val < 55 and vol_ok):
            return None
        direction = "SHORT"
