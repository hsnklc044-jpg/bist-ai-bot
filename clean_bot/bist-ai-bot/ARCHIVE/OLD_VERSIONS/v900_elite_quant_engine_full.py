import yfinance as yf
import requests
import time
import pandas as pd
from datetime import datetime
import csv, os

# ================== AYARLAR ==================
TELEGRAM_TOKEN = "8434925197:AAG8EZlJVk7pLiZpNxzLd3VXNgOi6CmwZCk"
CHAT_ID = "1790584407"

SYMBOLS = ["TUPRS.IS","EREGL.IS","SISE.IS","THYAO.IS","ASELS.IS"]

INTERVAL = "1m"
PERIOD = "1d"

SCAN_SLEEP = 10

ACCOUNT_SIZE = 100000
RISK_PER_TRADE = 0.02
MAX_TOTAL_RISK = 0.05

SL_ATR = 1.0
TP_ATR = 2.0
TRAIL_ATR = 0.8

WICK_MIN = 0.3
VOL_SPIKE = 1.1

MAX_SCALE = 2

LOG_FILE = "trades.csv"

# ================== STATE ==================
positions = {}
total_pnl = 0
trade_count = 0
win_count = 0

# ================== TELEGRAM ==================
def send(msg):
    try:
        url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
        requests.post(url, data={"chat_id": CHAT_ID, "text": msg})
    except:
        pass

# ================== LOG ==================
def log_trade(row):
    file_exists = os.path.isfile(LOG_FILE)
    with open(LOG_FILE,"a",newline="",encoding="utf-8") as f:
        w = csv.writer(f)
        if not file_exists:
            w.writerow(["time","symbol","side","entry","exit","pnl"])
        w.writerow(row)

# ================== RISK ==================
def current_total_risk():
    return len(positions) * RISK_PER_TRADE

def calc_size(price, atr):
    risk_amount = ACCOUNT_SIZE * RISK_PER_TRADE
    sl_distance = atr * SL_ATR
    if sl_distance == 0:
        return 0
    size = risk_amount / (sl_distance * price)
    return round(size, 2)

# ================== İNDİKATÖRLER ==================
def rsi(df):
    d = df['Close'].diff()
    gain = d.clip(lower=0).rolling(14).mean()
    loss = (-d.clip(upper=0)).rolling(14).mean()
    rs = gain / loss
    return 100 - (100/(1+rs))

def atr(df):
    tr = pd.concat([
        df['High']-df['Low'],
        (df['High']-df['Close'].shift()).abs(),
        (df['Low']-df['Close'].shift()).abs()
    ],axis=1).max(axis=1)
    return tr.rolling(14).mean()

# ================== DATA ==================
def get(symbol):
    try:
        df = yf.download(symbol, interval=INTERVAL, period=PERIOD, progress=False)
        if df is None or df.empty or len(df)<50:
            return None
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = df.columns.get_level_values(0)
        return df
    except:
        return None

# ================== ANALİZ ==================
def analyze(sym, df):
    df['rsi'] = rsi(df)
    df['atr'] = atr(df)

    last = df.iloc[-1]

    r = float(last['rsi'])
    o,c,h,l = float(last['Open']),float(last['Close']),float(last['High']),float(last['Low'])
    vol = float(last['Volume'])
    atr_v = float(last['atr'])

    rng = (h-l)+1e-9
    buy_w = (min(o,c)-l)/rng
    sell_w = (h-max(o,c))/rng

    avg_vol = df['Volume'].rolling(20).mean().iloc[-1]
    vol_ok = vol > avg_vol*VOL_SPIKE if pd.notna(avg_vol) else False

    score_buy = 0
    score_sell = 0

    if buy_w > WICK_MIN: score_buy+=2
    if r < 50: score_buy+=1
    if vol_ok: score_buy+=2

    if sell_w > WICK_MIN: score_sell+=2
    if r > 50: score_sell+=1
    if vol_ok: score_sell+=2

    if score_buy > score_sell and score_buy>=3:
        return {"sym":sym,"side":"LONG","price":c,"atr":atr_v,"score":score_buy}
    elif score_sell>=3:
        return {"sym":sym,"side":"SHORT","price":c,"atr":atr_v,"score":score_sell}
    return None

# ================== MAIN ==================
print("🚀 ELITE QUANT ENGINE FULL STARTED")
send("🧠 ELITE ENGINE FULL STARTED")

while True:
    try:
        print("\n⏱", datetime.now().strftime("%H:%M:%S"))

        for sym in SYMBOLS:
            df = get(sym)
            if df is None:
                continue

            sig = analyze(sym, df)
            if sig is None:
                continue

            price = sig["price"]
            atr_v = sig["atr"]

            # ================== EXIT ==================
            if sym in positions:
                pos = positions[sym]

                # LONG
                if pos["side"]=="LONG":

                    if price <= pos["entry"] - SL_ATR*pos["atr"]:
                        pnl = (price-pos["entry"])*pos["size"]
                        total_pnl += pnl
                        trade_count += 1
                        if pnl > 0: win_count += 1
                        print("🛑 LONG SL", pnl)
                        log_trade([datetime.now(),sym,"LONG",pos["entry"],price,pnl])
                        del positions[sym]
                        continue

                    if price >= pos["entry"] + TP_ATR*pos["atr"]:
                        pnl = (price-pos["entry"])*pos["size"]
                        total_pnl += pnl
                        trade_count += 1
                        if pnl > 0: win_count += 1
                        print("🎯 LONG TP", pnl)
                        log_trade([datetime.now(),sym,"LONG",pos["entry"],price,pnl])
                        del positions[sym]
                        continue

                    pos["trail"] = max(pos.get("trail", pos["entry"]), price)
                    if price < pos["trail"] - TRAIL_ATR*pos["atr"]:
                        pnl = (price-pos["entry"])*pos["size"]
                        total_pnl += pnl
                        trade_count += 1
                        if pnl > 0: win_count += 1
                        print("📉 TRAIL LONG", pnl)
                        log_trade([datetime.now(),sym,"LONG",pos["entry"],price,pnl])
                        del positions[sym]
                        continue

                # SHORT
                elif pos["side"]=="SHORT":

                    if price >= pos["entry"] + SL_ATR*pos["atr"]:
                        pnl = (pos["entry"]-price)*pos["size"]
                        total_pnl += pnl
                        trade_count += 1
                        if pnl > 0: win_count += 1
                        print("🛑 SHORT SL", pnl)
                        log_trade([datetime.now(),sym,"SHORT",pos["entry"],price,pnl])
                        del positions[sym]
                        continue

                    if price <= pos["entry"] - TP_ATR*pos["atr"]:
                        pnl = (pos["entry"]-price)*pos["size"]
                        total_pnl += pnl
                        trade_count += 1
                        if pnl > 0: win_count += 1
                        print("🎯 SHORT TP", pnl)
                        log_trade([datetime.now(),sym,"SHORT",pos["entry"],price,pnl])
                        del positions[sym]
                        continue

                    pos["trail"] = min(pos.get("trail", pos["entry"]), price)
                    if price > pos["trail"] + TRAIL_ATR*pos["atr"]:
                        pnl = (pos["entry"]-price)*pos["size"]
                        total_pnl += pnl
                        trade_count += 1
                        if pnl > 0: win_count += 1
                        print("📉 TRAIL SHORT", pnl)
                        log_trade([datetime.now(),sym,"SHORT",pos["entry"],price,pnl])
                        del positions[sym]
                        continue

                # SCALE IN
                if sig["side"] == pos["side"] and pos.get("scale",0) < MAX_SCALE:
                    pos["size"] *= 1.2
                    pos["scale"] = pos.get("scale",0) + 1
                    print("🔥 SCALE IN", sym)

                continue

            # ================== RISK CONTROL ==================
            if current_total_risk() >= MAX_TOTAL_RISK:
                print("⚠️ MAX RISK dolu")
                continue

            # ================== ENTRY ==================
            size = calc_size(price, atr_v)
            if size <= 0:
                continue

            positions[sym] = {
                "side": sig["side"],
                "entry": price,
                "size": size,
                "atr": atr_v,
                "trail": price
            }

            winrate = (win_count/trade_count*100) if trade_count>0 else 0

            msg = (
                f"{'🟢 LONG' if sig['side']=='LONG' else '🔴 SHORT'} {sym}\n"
                f"Price:{price:.2f}\nSize:{size}\n"
                f"PnL:{total_pnl:.2f}\nWinrate:{winrate:.1f}%"
            )

            print("🚀", msg)
            send(msg)

        time.sleep(SCAN_SLEEP)

    except Exception as e:
        print("❌ hata", e)
        time.sleep(SCAN_SLEEP)