...
# ================= MARKET TYPE =================
def market_trending(close):
    e20 = ema(close,20)
    e50 = ema(close,50)

    distance = abs(e20.iloc[-1] - e50.iloc[-1]) / close.iloc[-1]

    # güçlü trend varsa true
...
def breakout(close):
    ...
    return current > last_high * 1.002

# ================= RUN =================
def run():
    send_telegram("🚀 V60 AI MARKET AKTİF")
    ...
    while True:
        try:
            for symbol in COINS:
                ...
                r = rsi(c).iloc[-1]
                t = trend(c).iloc[-1]
                market_ok = market_trending(c)

                print(symbol,"RSI:",round(r,2),"Trend:",t,"Market:",market_ok)
