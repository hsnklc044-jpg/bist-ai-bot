...
def run():
    ...
    while True:
        ...
        for sym in list(state.keys()):
            ...
            result = check_close(sym, t)
            if result:
                typ, price_close = result

                pnl = ((price_close - t["entry"]) / t["entry"]) * 100
                if t["side"]=="SHORT":
                    pnl *= -1

                send(f"{'🎯 TP' if typ=='TP' else '❌ SL'}\n{sym}\nPNL: {round(pnl,2)}%")
