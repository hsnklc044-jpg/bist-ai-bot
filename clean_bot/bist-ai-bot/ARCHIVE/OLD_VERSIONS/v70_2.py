...
def micro_move(df):
    return abs(df["close"].iloc[-1] - df["close"].iloc[-2]) > 0
...
def signal(symbol):
    ...
    # 🔥 SMART ENTRY
    if (sw and score >= 40) or (vol and micro and score >= 20):
        price = df5["close"].iloc[-1]
        send_telegram(f"🚀 V70.2 ENTRY {symbol} {price} | Score:{score}")
