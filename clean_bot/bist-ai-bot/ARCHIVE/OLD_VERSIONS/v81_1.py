...
def analyze(symbol):
    try:
        ...
        # 🔎 DEBUG LOG
        print(f"{symbol} | RSI:{round(rsi_val,2)} | TrendUp:{trend_up} | TrendDown:{trend_down} | MomUp:{mom_up} | MomDown:{mom_down} | Vol:{vol_ok}")
        ...
        if score < 60 or not vol_ok:
            return None

        # 🔥 PROFESYONEL RSI BAND
        if trend_up and mom_up and 52 < rsi_val < 58:
            direction = "LONG"
        elif trend_down and mom_down and 35 < rsi_val < 50:
            direction = "SHORT"
        else:
            return None
