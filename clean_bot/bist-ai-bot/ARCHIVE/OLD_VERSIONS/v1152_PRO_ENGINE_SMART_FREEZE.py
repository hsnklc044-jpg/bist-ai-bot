...
PRICE_THRESHOLD = 0.001  # %0.1 tolerans
...
def is_frozen(symbol, price, volume):
    if symbol not in last_data:
        last_data[symbol] = (price, volume)
        freeze_counter[symbol] = 0
        return False

    last_price, last_volume = last_data[symbol]

    price_change = abs(price - last_price) / last_price
    volume_change = volume - last_volume

    # 🔥 SMART FREEZE LOGIC
    if price_change < PRICE_THRESHOLD and volume_change == 0:
...
print("🚀 SMART FREEZE ENGINE STARTED")
