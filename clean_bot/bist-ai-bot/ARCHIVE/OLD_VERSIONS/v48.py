...
print("V48 KURUMSAL BOT BASLADI...")

while True:
    try:
        # 🔥 GÜNLÜK RAPOR
        if date.today() != last_report_day:
            send_telegram(f"📊 RAPOR\nWin:{wins} Loss:{losses}")
            last_report_day = date.today()

        # 🔥 RİSK KORUMA
        if consec_loss >= MAX_CONSEC_LOSS:
            send_telegram("🛑 SISTEM DURDU (ust uste zarar)")
            time.sleep(60)
            continue
        ...
        # ===== ÇIKIŞ =====
            ...
            if position_side == "BUY":
                ...
                elif price >= max_profit_price * 1.01:
                    place_order(active_symbol, "BUY", position_qty)
                    send_telegram(f"💰 WIN {active_symbol}")
                    wins += 1
                    consec_loss = 0
