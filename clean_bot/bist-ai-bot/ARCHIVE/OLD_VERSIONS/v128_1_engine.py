...
def get_signal(symbol):
    ...
    print(f"{symbol} TREND:{trend} EMA9:{round(e9,2)} EMA21:{round(e21,2)} RSI:{round(r,2)}")

    # CHOP FILTER (hafif gevşetildi)
    if abs(e9 - e21) / e21 < 0.0007:
        print("BLOCK: chop")
        return None
    ...
    if trend == "LONG" and e9 > e21 and r > 52:
        return "LONG"

    if trend == "SHORT" and e9 < e21 and r < 48:
        return "SHORT"
...
def portfolio_ok(side):
    ...
    print(f"RISK CHECK → active:{len(active)} same:{len(same)}")
...
# ================= ENGINE ===================
def run():
    print("🚀 v128.1 PRO (OPTIMIZED SYSTEM)")
