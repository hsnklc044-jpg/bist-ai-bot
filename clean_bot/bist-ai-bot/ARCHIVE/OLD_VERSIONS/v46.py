    ...
    try:
        ...
        # ===== POZİSYON YÖNETİMİ =====
            ...
            if position_side == "LONG":
                ...
                sl = entry_price * 0.98
                trailing_sl = max_profit_price * 0.99

                if price <= sl:
                    place_order("SELL", qty())
                    send_telegram(f"❌ STOP LOSS LONG {price}")
                    in_position = False

                elif price <= trailing_sl:
                    place_order("SELL", qty())
                    send_telegram(f"💰 TRAILING STOP LONG {price}")
                    in_position = False
                ...
                sl = entry_price * 1.02
                trailing_sl = max_profit_price * 1.01

                if price >= sl:
                    place_order("BUY", qty())
                    send_telegram(f"❌ STOP LOSS SHORT {price}")
                    in_position = False

                elif price >= trailing_sl:
                    place_order("BUY", qty())
                    send_telegram(f"💰 TRAILING STOP SHORT {price}")
                    in_position = False
