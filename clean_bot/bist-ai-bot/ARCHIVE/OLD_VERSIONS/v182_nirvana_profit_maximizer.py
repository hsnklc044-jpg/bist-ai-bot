...
def manage_trades():
    for s in list(state.keys()):
        ...
        # 🔒 BREAK EVEN
        if "be" not in t:
            if side=="LONG" and p > entry * 1.005:
                t["sl"] = entry
                t["be"] = True
                send(f"🔒 BE ACTIVATED {s}")
            elif side=="SHORT" and p < entry * 0.995:
                t["sl"] = entry
                t["be"] = True
                send(f"🔒 BE ACTIVATED {s}")
...
# ================= MAIN =================
def run():
    print("🚀 v182 PROFIT MAXIMIZER")
    send("✅ BOT STARTED v182")
