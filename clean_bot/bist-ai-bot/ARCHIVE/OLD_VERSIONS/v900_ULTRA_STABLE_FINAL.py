...
def get(sym):
    try:
        ...
        if df is None or df.empty:
            print(f"⚠️ DATA YOK: {sym}")
            return None

        if len(df) < 50:
            print(f"⚠️ YETERSİZ DATA: {sym}")
            return None
        ...
        for col in required:
            if col not in df.columns:
                print(f"⚠️ EKSİK KOLON: {sym}")
                return None

        df = df.dropna()

        if len(df) < 50:
            print(f"⚠️ NaN sonrası yetersiz: {sym}")
            return None
...
# ================= MAIN =================
print("🚀 ULTRA STABLE SYSTEM STARTED")
send("🚀 ULTRA STABLE SYSTEM STARTED")
