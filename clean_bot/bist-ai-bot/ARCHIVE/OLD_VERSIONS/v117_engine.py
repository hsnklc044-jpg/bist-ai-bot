...
def signal(df, symbol):
    ...
    if trend == "UP":
        ...
        if prev['close'] > breakout:

            # retest toleransı (zorunlu değil)
            if abs(prev['low'] - breakout) / breakout < 0.002:
    ...
    if trend == "DOWN":
        ...
        if prev['close'] < breakout:
            ...
            if abs(prev['high'] - breakout) / breakout < 0.002:
...
def manage(df):
    ...
    if open_trade['side']=="LONG":
        ...
        if not open_trade['be'] and price > entry*1.003:
            open_trade['sl'] = entry
            open_trade['be'] = True
            send("🔒 BE LONG")

        if price > entry*1.007:
            open_trade['sl'] = max(open_trade['sl'], price*0.997)
    ...
    if open_trade and open_trade['side']=="SHORT":
        ...
        if not open_trade['be'] and price < entry*0.997:
            open_trade['sl'] = entry
            open_trade['be'] = True
            send("🔒 BE SHORT")

        if price < entry*0.993:
            open_trade['sl'] = min(open_trade['sl'], price*1.003)
