...
# ================= MAIN =================
def run():
    print("🚀 v158 ELITE BALANCED SYSTEM")

    while True:

        # ===== CLOSE =====
        for sym in list(state.keys()):
            if sym == "last_trade_time":
                continue
        ...
        # ===== TEK TRADE =====
        if len([k for k in state if k!="last_trade_time"]) > 0:
        ...
        # ===== SMART FALLBACK v2 (BALANCED) =====
        LOW_SCORE_MODE = False

        if score < MIN_SCORE:
            if score < 0.028:
                print("Rejected: ultra weak")
                time.sleep(LOOP_DELAY)
                continue
        ...
        risk = 0.005 if LOW_SCORE_MODE else 0.01
        qty = size(entry,sl,risk)

        send(f"""🏆 ELITE BALANCED TRADE
{s} {sig}
...
Mode: {"LOW RISK" if LOW_SCORE_MODE else "NORMAL"}
Score:{round(score,5)}""")
