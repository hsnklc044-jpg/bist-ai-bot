...
def manage_trade(symbol, price):
    ...
    # 🎯 HALF TP
    if not t["half_closed"] and price >= t["entry"] + (t["tp"] - t["entry"]) * 0.5:
        t["half_closed"] = True
        t["sl"] = t["entry"]  # BE
        send_telegram(f"💰 HALF TP {symbol} → BE AKTİF")

    # 🔥 TRAILING
    if t["half_closed"]:
        new_sl = price - (price * 0.01)
        if new_sl > t["sl"]:
            t["sl"] = new_sl

    # TP
    if price >= t["tp"]:
        send_telegram(f"✅ FULL TP {symbol}")
...
# ================= RUN =================
def run():
    send_telegram("🚀 V64 PRO ENGINE AKTİF")
